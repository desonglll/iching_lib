...
use iching_lib::shi_chen::get_current_season_pro;
...
fn main() {
    ...
    let shichen_num = 4;
    let season = get_current_season_pro();
    let now = Local::now();
    ...
    print!(
        "六十四卦：\t{}\t",
        ben_gua.to_liu_shi_si_gua().unicode_symbol()
    );
    ...
    print!(
        "六十四卦：\t{}\t",
        bian_gua.to_liu_shi_si_gua().unicode_symbol()
    );
    ...
    print!(
        "六十四卦：\t{}\t",
        hu_gua.to_liu_shi_si_gua().unicode_symbol()
    );
    ...
    let bian = ben_gua.bian_gua(dong_yao).unwrap();
    // 变卦中，“用”卦是发生了变动的那部分
