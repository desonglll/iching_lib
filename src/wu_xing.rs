...
impl WuXing {
    ...
    pub fn get_energy_status(&self, season: u8) -> &'static str {
        use WuXing::*;
        match (season, self) {
            (1, Mu) => "旺（最强）", (1, Huo) => "相（次强）", (1, Shui) => "休", (1, Tu) => "囚", (1, Jin) => "死",
            (2, Huo) => "旺（最强）", (2, Tu) => "相（次强）", (2, Mu) => "休", (2, Shui) => "囚", (2, Jin) => "死",
            (3, Jin) => "旺（最强）", (3, Shui) => "相（次强）", (3, Tu) => "休", (3, Huo) => "囚", (3, Mu) => "死",
            (4, Shui) => "旺（最强）", (4, Mu) => "相（次强）", (4, Jin) => "休", (4, Tu) => "囚", (4, Huo) => "死",
            _ => "平（土旺）",
        }
    }
}